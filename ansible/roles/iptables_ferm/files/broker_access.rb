...
class IpsetBroker
  ...
  def initialize(opts)
    ...
    @ipset_name = opts.ipset_name || 'broker_access'
    @log = init_logger(opts.logoutput || STDOUT)
    @dry_run = opts.dry_run || false
  end

  def main
    @log.debug "Start broker IPs update"
    update_ipset(@action,http_brokerlist,@ipset_name)
  end
...
private
  ...
  def update_ipset(action,brokerlist,name)
    flush_ipset(name)
      brokerlist.each do |ip|
        @log.info "Add broker ip: #{ip}"
        ipset('add',name,ip)
      end
  end
  ...
  def get_brokerlist_from_json(http_data)
    list = []
    ...
    if !json_data.empty?
      json_data.each do |entry|
        if !entry_expired?(curr_time, entry)
          list << entry['ip']
          @log.debug "Entry add to list: #{entry['ip']} (valid until: \"#{entry['expiration']}\", #{entry['comment']})"
        else
  ...
  def http_brokerlist
    ...
    @log.debug "Broker list url: #{uri}"
    if res.is_a?(Net::HTTPSuccess)
      list = get_brokerlist_from_json(res.body)
    else
      raise "Request to #{uri} with code #{res.code} #{res.body}"
    end
    @log.debug "Broker IP list: #{list}"
    return list
  end
end

class IpsetBrokerOptionParser
  ...
  def self.parse(args)
    ...
    opt_parser = OptionParser.new do |opts|
      ...
      opts.on("-u", "--url [URL]", "Url with broker IP list in json format") do |url|
        options.url = url
      end
...
if __FILE__ == $0
  options = IpsetBrokerOptionParser.parse(ARGV)
  bh = IpsetBroker.new(options)
  bh.main
end
